...
### color scheme 
snp_indel_sv_vrnt_colors <- c("snp"="#DC853E", 
                           "indel"="#44AA99", 
                           "all_sv"="#332288")

snp_indel_vrnt_colors <- c("snp"="#DC853E", 
                           "indel"="#44AA99")
...
# Supplementary Fig. 9
supp_fig9_out_dir <- file.path(wkdir, "figs/supp_fig_9")
dir.create(file.path(supp_fig9_out_dir))
# Supplementary Fig. 10
supp_fig10_out_dir <- file.path(wkdir, "figs/supp_fig_10")
dir.create(file.path(supp_fig10_out_dir))
# Supplementary Fig. 11
supp_fig11_out_dir <- file.path(wkdir, "figs/supp_fig_11")
dir.create(file.path(supp_fig11_out_dir))
```
...
snv_indel_sv_results_path <- file.path(wkdir, "4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/snp_indel_sv_gene_window_results.tsv")
...
snv_indel_sv_enrich_gene_body <- ggplot(snv_indel_sv_z_cutoff_results_df, aes(x=z_cutoff_name, y=risk_ratio, color=vrnt_type)) + 
  ...
  scale_y_continuous(name = "Enrichment", breaks=c(1, 10, 20, 30), limits=c(0, 32), expand=c(0.1,0.1)) +
...
```
### Percentage misexpression carriers at different z-score cuttoffs (+/- 200kb window)
```{r}
perc_misexp_carrier_path <- file.path(wkdir, "4_vrnt_enrich/combine_count_carriers/perc_misexp_carrier_zscores_200kb.tsv")
...
width=6
height=5
perc_misexp_carrier_zscore_pdf = file.path(supp_fig6_out_dir, "perc_misexp_carrier_zscore_200kb.pdf")
...
dev.off()
theme.perc_misexp_carrier(perc_misexp_carrier_fig)
perc_misexp_carrier_zscore_png <- file.path(supp_fig6_out_dir, "perc_misexp_carrier_zscore_200kb.png")
ggsave(perc_misexp_carrier_zscore_png, width=width, height=height, dpi=360)
```
```{r}
perc_misexp_carrier_path <- file.path(wkdir, "4_vrnt_enrich/combine_count_carriers/perc_misexp_carrier_zscores_10kb.tsv")
perc_misexp_carrier_df <- read.csv(perc_misexp_carrier_path, sep="\t")
...
width=6
height=5
perc_misexp_carrier_zscore_pdf = file.path(supp_fig6_out_dir, "perc_misexp_carrier_zscore_10kb.pdf")
pdf(file=perc_misexp_carrier_zscore_pdf, width = width, height = height, useDingbats=FALSE)

perc_misexp_carrier_fig <- ggplot(perc_misexp_carrier_df, aes(y=perc_carrier, x=z.score)) +
   geom_bar(stat = "identity", fill = "#5B84B1FF", alpha = 0.5, color="black", size=ticks_size) + 
   scale_y_continuous(name = "% misexpression events with SV within 10 kb", limits=c(0,3), expand=c(0,0)) + 
   scale_x_discrete(name = "Misexpression z-score threshold", expand=c(0.1, 0.1)) + 
   geom_text(aes(label = round(misexp_carrier, 1)), vjust =-0.5, size = 4.5, fontface="plain", color="black")
theme.perc_misexp_carrier(perc_misexp_carrier_fig)
dev.off()
theme.perc_misexp_carrier(perc_misexp_carrier_fig)
perc_misexp_carrier_zscore_png <- file.path(supp_fig6_out_dir, "perc_misexp_carrier_zscore_10kb.png")
ggsave(perc_misexp_carrier_zscore_png, width=width, height=height, dpi=360)
```

### SNV, and indel, MAF and z-score cutoffs (Zoomed in plot of SNVs and indels only)
```{r}
snv_indel_z_nolog_path <- file.path(supp_fig7_out_dir, "snv_indel_enrich_gene_body_zoom.pdf")
pdf(snv_indel_z_nolog_path, width = 9, height = 3.5, useDingbats=FALSE)

snv_indel_enrich_gene_body <- ggplot(snv_indel_sv_z_cutoff_results_df[snv_indel_sv_z_cutoff_results_df$vrnt_type %in% {c("snp", "indel")},],
                                        aes(x=z_cutoff_name, y=risk_ratio, color=vrnt_type)) + 
  geom_hline(yintercept=1.0, linetype="solid", size=ticks_size) + 
  geom_point(position = position_dodge(width = 0.6), size=3) + 
  geom_errorbar(aes(ymin=risk_ratio_lower, ymax=risk_ratio_upper),cex=1, width=0, position = position_dodge(width=0.6)) + 
  scale_x_discrete(name = "Misexpression z-score threshold", labels=c("2", "10", "20", "30", "40", "50")) +
  scale_y_continuous(name = "Enrichment", breaks=c(0.8, 1, 1.2, 1.4, 1.6), limits=c(0.75, 1.5), expand=c(0,0)) +
  scale_color_manual(name = '', 
                    labels=snv_indel_labeller,
                    values= snp_indel_vrnt_colors, 
                    ) +
  facet_grid(.~maf_range, scales = "free", space = "free", switch='y', labeller = as_labeller(facet_labels)) + 
  geom_text(aes(label = ifelse(bonferroni_pass  == "True"  & risk_ratio > 1, "*", ""), y=risk_ratio_upper), hjust = 0.5, vjust = -0.25, position = position_dodge(width = 0.6), size=4.5, fontface="bold", show.legend  = FALSE) + 
  geom_text(aes(label = ifelse(bonferroni_pass  == "True" & risk_ratio < 1, "*", ""), y=risk_ratio_lower), hjust = 0.5, vjust = 1.75, position = position_dodge(width = 0.6), size=4.5, fontface="bold", show.legend  = FALSE) + 
...
theme.snv_indel_sv_maf_z(snv_indel_enrich_gene_body)
dev.off()
theme.snv_indel_sv_maf_z(snv_indel_enrich_gene_body)
snv_indel_z_path_nolog_png <- file.path(supp_fig7_out_dir, "snv_indel_enrich_gene_body_zoom.png")
ggsave(snv_indel_z_path_nolog_png, width=9, height=3.5, dpi=360)
```
...
snp_indel_sv_window_bins_path <- file.path(wkdir,"4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/snp_indel_sv_maf_0_1perc_window_bin_results.tsv")
...
snv_indel_sv_z_window_z_cutoffs_path = file.path(supp_fig8_out_dir, "snv_indel_sv_enrich_windows_z_cutoffs.pdf")
...
ggtitle("") +
geom_errorbar(aes(ymin=risk_ratio_lower, ymax=risk_ratio_upper),cex=1, width=0, position = position_dodge(width = 0.6)) + 
scale_x_discrete(name = "Genomic window") + 
scale_y_continuous(name = "Enrichment", 
                   expand=c(0.2,0.2)
                  ) + 
scale_colour_manual(name = 'Variant\n  Type', 
...
dev.off()
theme.misexp_snv_indel_windows_z(snv_indel_sv_window_z_cutoffs)
snv_indel_sv_enrich_windows_z_cutoffs_png <- file.path(supp_fig8_out_dir, "snv_indel_sv_enrich_windows_z_cutoffs.png")
ggsave(snv_indel_sv_enrich_windows_z_cutoffs_png, width=9, height=10, dpi=360)
```
### SNV and indel variant enrichment across windows and multiple z-score thresholds (zoomed in on SNVs and indels only)
```{r}
snv_indel_z_window_z_cutoffs_path = file.path(supp_fig9_out_dir, "snv_indel_enrich_windows_z_cutoffs_zoom.pdf")
pdf(snv_indel_z_window_z_cutoffs_path, width=9, height=10, useDingbats=FALSE)

snv_indel_window_z_cutoffs <- ggplot(snp_indel_sv_window_bins_z_subset_df[snp_indel_sv_window_bins_z_subset_df$vrnt_type %in% c("snp", "indel"),], 
                                     aes(x=factor(window_name, level=level_order), y=risk_ratio, color=vrnt_type)) +
    geom_hline(yintercept=1.0, linetype="solid", size=ticks_size) + 
geom_point(position = position_dodge(width = 0.6), size=3) +
ggtitle("") +
geom_errorbar(aes(ymin=risk_ratio_lower, ymax=risk_ratio_upper),cex=1, width=0, position = position_dodge(width = 0.6)) + 
...
scale_colour_manual(name = 'Variant\n  Type', 
                    ...
                    labels=snv_indel_sv_labeller,
                    guide="none") +
  facet_grid(z_cutoff_name~., space="fixed") + 
  geom_text(aes(label = ifelse(bonferroni_pass  == "True"  & risk_ratio > 1, "*", ""), y=risk_ratio_upper), hjust = 0.5, vjust = -0.25, position = position_dodge(width = 0.6), size=4.5, fontface="bold", show.legend  = FALSE) + 
  geom_text(aes(label = ifelse(bonferroni_pass  == "True" & risk_ratio < 1, "*", ""), y=risk_ratio_lower), hjust = 0.5, vjust = 1.75, position = position_dodge(width = 0.6), size=4.5, fontface="bold", show.legend  = FALSE) + 
  guides(fill = guide_legend(override.aes = list(label = NULL)))

theme.misexp_snv_indel_windows_z(snv_indel_window_z_cutoffs)
dev.off()
theme.misexp_snv_indel_windows_z(snv_indel_window_z_cutoffs)
snv_indel_enrich_windows_z_cutoffs_png <- file.path(supp_fig9_out_dir, "snv_indel_enrich_windows_z_cutoffs_zoom.png")
ggsave(snv_indel_enrich_windows_z_cutoffs_png, width=9, height=10, dpi=360)
```
...
sv_types_enrich_results_all_path = file.path(wkdir, "4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/sv_types_maf_0_1perc_200kb_all_z_results.tsv")
...
width = 8 
height = 4
rare_inv_mei_type_enrich_path = file.path(supp_fig10_out_dir, "rare_inv_mei_enrich_gene_body.pdf")
...
dev.off()
theme.sv_class_enrich(inv_mei_enrich)
rare_inv_mei_type_enrich_png <- file.path(supp_fig10_out_dir, "rare_inv_mei_enrich_gene_body.png")
...
sv_types_window_enrich_results_path <- file.path(wkdir, "4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/sv_types_maf_0_1perc_windows_all_z_results.tsv")
...
rare_sv_type_enrich_z_cutoffs_path = file.path(supp_fig10_out_dir, "rare_sv_types_enrich_windows_z_cutoffs.pdf")
...
dev.off()
theme.sv_class_window_enrich_supp(sv_class_window_enrich_fig)
rare_sv_type_enrich_z_cutoffs_path_png <- file.path(supp_fig10_out_dir, "rare_sv_types_enrich_windows_z_cutoffs.png")
...
# z-cutoff 
sv_msc_enrich_results_path <- file.path(wkdir,'4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/sv_consq_pass_bonf_z10.tsv')
...
sv_msc_enrich_results_path <-file.path(wkdir,'4_vrnt_enrich/enrich_results_mul_test/grouped_enrich_gene_body_10kb/sv_consq_pass_bonf.tsv')
...
rare_sv_msc_type_enrich_bonf_pass_all_zcutoff_pdf_path = file.path(supp_fig11_out_dir, "rare_sv_consq_by_gene_z10_200kb_gene_bonf_pass_any_all_zcutoff.pdf")
...
dev.off()
theme.sv_class_enrich(sv_msc_all_enrich)
rare_sv_msc_type_enrich_bonf_pass_all_zcutoff_png_path = file.path(supp_fig11_out_dir, "rare_sv_consq_by_gene_z10_200kb_gene_bonf_pass_any_all_zcutoff.png")
